...
class VoiceNotesApp {
  ...
  private copyFallbackButton!: HTMLButtonElement;
  ...
  constructor() {
    ...
    if (!apiKey) {
      alert('API key is required. Please set GEMINI_API_KEY in localStorage or environment.');
      return;
    }
    ...
    this.copyFallbackButton = document.getElementById(
      'copyFallbackButton',
    ) as HTMLButtonElement;
  ...
  private bindEventListeners(): void {
    ...
    window.addEventListener('mousedown', this.handleMiddleMouseClick.bind(this));
  }
  
  private handleMiddleMouseClick(event: MouseEvent): void {
  ...
  private async getPolishedNote(): Promise<void> {
    ...
    try {
      ...
      const prompt = `Take this raw transcription and create a polished, well-formatted note.
                    ...
                    IMPORTANT: Do not add any introductory or concluding remarks, such as "Here is the polished version".
                    Only return the polished note content itself, starting directly with the title or first line.
      ...
      if (polishedText) {
        ...
        // Proactively check if the document is focused. If not, don't even try to auto-copy.
        if (document.hasFocus()) {
          try {
            await navigator.clipboard.writeText(polishedText);
            this.recordingStatus.textContent = 'Note polished & copied. Ready for next recording.';
          } catch (err) {
            console.error('Failed to copy text to clipboard:', err);
            // This fallback is for when the document is focused but permission is still denied.
            this.showCopyFallback(polishedText);
          }
        } else {
          // This fallback is for when the document is not focused.
          this.showCopyFallback(polishedText);
        }
  ...
  private showCopyFallback(textToCopy: string): void {
    this.recordingStatus.textContent = 'Automatic copy failed. Click to copy.';
    this.copyFallbackButton.classList.remove('hidden');

    this.copyFallbackButton.onclick = () => {
        navigator.clipboard.writeText(textToCopy).then(() => {
            this.copyFallbackButton.innerHTML = '<i class="fas fa-check"></i> <span>Copied!</span>';
            this.recordingStatus.textContent = ''; 

            setTimeout(() => {
                this.recordingStatus.textContent = 'Ready for next recording.';
                this.hideCopyFallback();
            }, 2000);

        }).catch(copyErr => {
            console.error('Fallback copy failed:', copyErr);
            this.recordingStatus.textContent = 'Copy failed. Please copy manually.';
            this.hideCopyFallback();
        });
    };
  }

  private hideCopyFallback(): void {
      if (this.copyFallbackButton) {
          this.copyFallbackButton.classList.add('hidden');
          this.copyFallbackButton.onclick = null; // Remove handler
          this.copyFallbackButton.innerHTML = '<i class="fas fa-copy"></i> <span>Copy</span>';
      }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  const voiceNotesApp = new VoiceNotesApp();
  
  // Make methods available globally for Electron shortcuts
  (window as any).voiceNotesApp = {
    toggleRecording: () => voiceNotesApp.toggleRecording(),
    handleNewNote: () => voiceNotesApp.createNewNote()
  };
